...
function bind(htmlFunction, props) {
	...
	element.data = deepProxy(props, (target, prop, value, receiver) => {
		...
		setElement(newElement, expressions);
...
function html(templateString, ...expressions) {
	const html = parseTemplateString(templateString, expressions);
	const element = createElement(html);
	
	createForElements(element, expressions);
	setElement(element, expressions);
	setEvents(element, expressions);
...
function parseTemplateString(templateString, expressions) {
...
function createForElements(element, expressions) {
...
function setElement(element, expressions) {
	// value-selected
	element.parentNode.querySelectorAll('[value-selected]').forEach(input => {
		let selectedValue = expressions[input.getAttribute('value-selected')];

		if (input.type == 'radio') {
			input.checked = input.value == selectedValue;
		} else if (input.type == 'checkbox') {
			//input.checked = selectedValue.match(input.value);
		}
	});
}

function setEvents(element, expressions) {
	...
	function onEvent(event) {
		...
		const exprIndex = Number(target.expressionIndex);
		const result = expressions[exprIndex](event);
